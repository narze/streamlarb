<script lang="ts">
  ...
  function removeWindow() {
    ...
    const webview = getAll().find((w) => w.label === "theUniqueLabel")
...
<main class="w-full h-screen">
  {#if editMode}
    <div class="fixed top-10 left-10 w-full h-full text-lg">
      ...
      <p class="mt-4">
        <button on:click={addNewWindow} class="bg-red-300 px-2 py-1 rounded"
          >Add new window</button
        >
      </p>
      <p class="mt-4">
        <button on:click={removeWindow} class="bg-red-300 px-2 py-1 rounded"
